...
## Day 18 — classification 승격 구현·검증 (2026-08-17)

- **승격 구현 (커밋 `5b45211`, Oracle 스펙 A/B/C/D)**:
  - `filing_date_used` 하드 증명: `_convert_financial_to_daily`가
    `filing_date_mapped_rows`/`quarter_end_fallback_rows` 실측 카운트 →
    attrs 및 `PreparedK200MQInputs.financial_filing_date_mapped_row_count`
    전달. strict preflight는 측정 카운트 사용(레거시 None이면 기존
    `has_usable_filing_dates` 폴백).
  - guard 완화: `walk_forward.py:579` deferral raise 제거,
    `select_candidate`에 `pit_valid_evidence` 요구(문자열 단독 승격 불가
    불변식 유지), `runner.py`는 Mapping `pit_valid_context`만 수용·
    스레딩·freeze.
  - adapter(`main.py:3094-3142`): strict preflight 후 유니버스/재무 validator
    재실행 → `strict_pit && universe_ok && financial_ok`일 때만 validated,
    invalid 결과는 아티팩트 저장 전 차단(누수 가드).
  - ADV 필터: 커버리지 누락 티커(상장폐지 mcap=0)는 경고+제외로 진행
    (None 맵은 여전히 fail-closed) — Day 15 PIT 실패 해소.
- **증거 품질 수정 (커밋 `4da34ad`)**: coverage_summary가 `records[0]`
  (2015-01-30, 0/200) 대신 `first_ready_rebalance.scheduled_date` 매칭
  record(2015-05-29, 73/200=36.5%) 사용; validated limitations에서
  "Mechanical non-PIT" 문구 제거 → validated 문구 + 실측 커버리지 명시.
- **Day 18 검증 (`outputs_k200mq_day18_validation_check_v2`)**:
  - 최초로 **classification=`validated_expanding_walk_forward_pit`** 승격.
    valid=True, 5/5 폴드, OOS 1,231점, preflight 실패 0건.
  - coverage_summary: `{"first_rebalance": {"six_fact_ratio": 0.365,
    "momentum_ratio": 0.605}}` (73/200, 121/200 실측).
  - claim: "validated PIT walk-forward; universe + financial provenance
    validators passed". limitations: validated 문구 + 실측 커버리지
    (36.5%/60.5%) 명시 — "Mechanical non-PIT" 잔존 없음.
  - stitched OOS **+20.55%**, Day 14와 폴드별 완전 동일 — 승격 코드가 성과
    로직에 영향 없음 (라벨 정직성 개선).
- **scorecard**: `08_go_no_go_scorecard_2026-08-16.md` **Hold** — 데이터
  게이트 완성, 성과 게이트 미달(OOS CAGR/Sharpe/Calmar 기준 이하, 2020
  서브기간 편중). classification 승격은 판정 변경이 아님.
...
## Day 21 — 파라미터 안정성 그리드 (2026-08-17, validated, PIT 유니버스)

- 실행기: `scripts/run_parameter_grid.sh` — 8개 조합, 모두
  `validated_expanding_walk_forward_pit`, 5/5 폴드 valid.
- 결과 (stitched OOS 2020-2024, stitched 순):

| config | stitched | CAGR | Sharpe | MDD | Calmar | 후보 선택 |
|---|---|---|---|---|---|---|
| **sl20 (0.7/0.3, SL-20%)** | **+58.48%** | 9.66% | 0.567 | -28.37% | **0.340** | REGIME_OFF 5 |
| Day20 (0.7/0.3, SL-15%) | +53.23% | 8.92% | 0.555 | -31.16% | 0.286 | REGIME_OFF 5 |
| mom06 (0.6/0.4) | +37.14% | 6.53% | 0.438 | -33.97% | 0.192 | REGIME_OFF 5 |
| cash10 (0.7/0.3, cash10%) | +33.18% | 5.91% | 0.508 | **-22.55%** | 0.262 | MOM60 4, REGIME_OFF 1 |
| mom08 (0.8/0.2) | +32.33% | 5.77% | 0.486 | -23.55% | 0.245 | MOM60 4, REGIME_OFF 1 |
| rebalQ (0.7/0.3, Q) | 27.40% | 4.97% | 0.407 | -27.04% | 0.184 | MOM60 3, REGIME_OFF 2 |
| pos15 (0.7/0.3, wgt15%) | 27.40% | 4.97% | 0.407 | -27.04% | 0.184 | MOM60 3, REGIME_OFF 2 |
| Day18 (0.5/0.5) | +20.55% | 3.81% | 0.299 | -37.68% | 0.101 | REGIME_OFF 5 |
| sl10 (0.7/0.3, SL-10%) | +13.89% | 2.64% | 0.257 | -30.89% | 0.085 | 혼합 |
| mom04 (0.4/0.6) | +0.40% | 0.08% | 0.094 | -44.62% | 0.002 | REGIME_OFF 5 |

- **유효성 발견 2건 (그리드 해석 전제)**:
  1. **rebalQ는 분기 테스트가 아님** — PIT 스냅샷 번들 import
     (`pit_universe.py:_import_local_pit_snapshot_bundle`)가 각 스냅샷을
     `requested_rebalance_dates=None`으로 개별 import 후 concat하므로
     REBALANCE_FREQ=Q가 무시되고 120개 월말 스냅샷 전부 사용
     (rebalQ date_count=120 확인). rebalQ와 pos15의 oos_returns가
     **완전 동일** — 두 변형 모두 무효 (월간 기본과 같음).
  2. **pos15도 무효** — 20종목 등액 가중(5%/종목)은 기본 MAX_POSITION_WEIGHT
     0.10 캡에 걸리지 않아 0.15로 올려도 변화 없음.
- 해석 (유효한 6개 변형 기준):
  - **모멘텀 가중 0.7이 최적** (0.4 붕괴 +0.40% → 0.6 +37.14% → 0.7 +53.23%
    → 0.8 +32.33%). WFA 후보 라이브러리 확장(MOM60/70, v3)으로 가중 축이
    train에서 선택됨 — mom08/cash10에서 MOM60이 4폴드 선택된 것 확인.
  - **손절 -20%가 -15%보다 개선** (+58.48%, Calmar 0.340 — 최고). 손절 -10%는
    오히려 악화 (+13.89%). 손절 임계값 상향이 유효한 조정 후보.
  - **현금 버퍼 10%는 MDD 최선** (-22.55%)이나 성과 희생 — MDD 게이트(-25%)
    통과 유일 조합. 리스크 선호에 따라 후보.
  - 2020 서브기간 편중은 모든 조합에서 지속 (2020 단일 연도가 성과 대부분).
...
- 체크리스트는 종료되었고, 최종 판정은 MQ 연구 중단·피벗이다.
- Iteration 3 섹터 30% cap, live trading, paper trading 및 현재 OOS 추가 튜닝은
  진행하지 않는다.
