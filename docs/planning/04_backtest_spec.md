...
> **보관·대체 공지 (2026-08-21)**: 이 문서는 K200 MQ 백테스트·검증의 역사적 사양
> 및 감사 참고용이다. MQ 연구는 중단·피벗되었고, 문서에 남은 미래 지향적 지침은
...
**Day 24 검증: REGIME_70/50/30은 모든 폴드에서 동일 train_sharpe 기록 —
축소 비율 조정이 성과에 영향 없음 (구조적 실패 확증).**
...
## PIT 입력 게이트와 보류 분석 (역사 기록)

당시 다음 게이트는 충분한 기간의 입력을 확보하고 운영 준비 경로에 연결하는 것이었습니다.
...
따라서 당시 다음 분석은 모두 보류 상태였습니다. 최종 결정 이후 실행하지 않습니다.
