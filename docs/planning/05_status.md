...
### 2026-08-22 KSD/FSC schema probe 기록 (dataset 15059609; Phase 2 완료 아님)
...
### 2026-08-22 OpenDART 기업행동 schema probe 기록 (Phase 2 완료 아님)
...
### 2026-08-22 저변동성 adjusted-price 진단 실행 결과 (ARCHIVE_PERMANENTLY)
...
- 결과 (adjusted-price 낙관적 상한): 연결 총수익률 **+8.20%** (~9.75년), **CAGR
  ...
  2023 +8.4%, 2024 +9.1%.
- **판정: ARCHIVE_PERMANENTLY** — 세 게이트 모두 결정적 실패(한계 근접 아님). adjusted
  price는 총수익률을 근사해 가격수익률 목표를 과대평가하므로, 검증된 가격수익률
  버전은 a fortiori 실패한다. 사전 규칙에 따라 신규 경제적 가설 부재 시 저변동성
  가설은 영구 보관된다.
- 상태: 저변동성 연구는 이제 **영구 보관**(기존에는 데이터 원천 대기 종료). MQ는
  보관·피벗 유지. 양쪽 모두 live trading, 추가 튜닝, OOS 반복 없음. Phase 0/1/2A
  code contract는 유효한 역사적 인프라로 유지.
...
- **REGIME_70/50/30 선택 0건**: 모든 폴드에서 BASE/REGIME_30/50/70이 동일
  train_sharpe — REGIME_REDUCTION 파라미터가 전략 결과에 영향 없음.
  이진 리짓 신호(close>MA200 AND 20d return>0)의 축소 비율 조정이
  성과에 기여하지 않음. **구조적 실패 확증.**
- **폴드별 선택**: Fold 1-3 REGIME_OFF / Fold 4-5 MOM60 (Day 22와 동일 패턴).
- **OOS 성과**: stitched **+16.41%**, CAGR 3.16%, Sharpe 0.274,
  MDD -37.99%, Calmar 0.083. Day 22 대비 모든 지표 악화.
